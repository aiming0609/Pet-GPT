...
class ChatWindow(QMainWindow):
    def __init__(self, parent=None,config="private_config.ini"):
        ...
        self.custom_dropdown.addItem("自定义选项 1")
    ...
    def python_slot(self):
        message = f'我希望你表现得像个Python解释器。我会给你Python代码，你会执行它。不要提供任何解释。' + \
            '除了代码的输出之外，不要使用任何东西进行响应。第一个代码是: "print(\'Hello World!\')"'
        self.send_to_gpt(message)
    ...
    def send_to_gpt(self,message):
        ...
        prompt = message
        self.chat_dialog_body.flowise.prompt_queue.put((prompt, self.chat_dialog_body.context_history,message,False))  # 将聊天上下文作为第二个参数传递

    # 为下拉按钮创建槽函数-模板
    def full_slot(self):
        selected_item = self.custom_dropdown.currentText()
        if selected_item == "自定义选项 1":
            self.function_1()
